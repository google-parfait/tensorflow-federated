...
*   The private member function `DPClosedDomainHistogram::CreateIthMechanism`:
    it makes either a `differential_privacy::LaplaceMechanism` or
    `differential_privacy::GaussianMechanism`, based upon privacy parameters and
    norm bounds.

### Changed

*   `DPClosedDomainHistogram::Report`: it now adds noise to each aggregate to be
    output, where the noise is generated by
    `DPClosedDomainHistogram::CreateIthMechanism`
