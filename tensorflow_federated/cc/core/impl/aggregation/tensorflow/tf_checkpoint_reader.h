...
// Vendored copy of tensorflow/c/checkpoint_reader.h, moved into the
...
#ifndef THIRD_PARTY_TENSORFLOW_FEDERATED_CC_CORE_IMPL_AGGREGATION_TENSORFLOW_TF_CHECKPOINT_READER_H_
#define THIRD_PARTY_TENSORFLOW_FEDERATED_CC_CORE_IMPL_AGGREGATION_TENSORFLOW_TF_CHECKPOINT_READER_H_
...
class CheckpointReader {
 public:
  ...
  const ::tensorflow::checkpoint::TensorSliceReader::VarToShapeMap&
  GetVariableToShapeMap() const;

  // Returns a map from variable names to their data types.  Slices of a
  // partitioned tensor are combined into a single entry.
  const ::tensorflow::checkpoint::TensorSliceReader::VarToDataTypeMap&
  GetVariableToDataTypeMap() const;
 ...
 private:
  ...
  std::pair<
      std::unique_ptr<
          ::tensorflow::checkpoint::TensorSliceReader::VarToShapeMap>,
      std::unique_ptr<
          ::tensorflow::checkpoint::TensorSliceReader::VarToDataTypeMap>>
  BuildV2VarMaps();

  // Invariant: exactly one of "reader_" and "v2_reader_" is non-null.
  std::unique_ptr<::tensorflow::checkpoint::TensorSliceReader> reader_;
  std::unique_ptr<::tensorflow::BundleReader> v2_reader_;

  std::unique_ptr<::tensorflow::checkpoint::TensorSliceReader::VarToShapeMap>
      var_to_shape_map_;
  std::unique_ptr<
      ::tensorflow::checkpoint::TensorSliceReader::VarToDataTypeMap>
...
#endif  // THIRD_PARTY_TENSORFLOW_FEDERATED_CC_CORE_IMPL_AGGREGATION_TENSORFLOW_TF_CHECKPOINT_READER_H_
