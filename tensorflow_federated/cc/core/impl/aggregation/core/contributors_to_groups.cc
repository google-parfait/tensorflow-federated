...
absl::Status ContributorsToGroups::AddCountToContributors(size_t group_index,
                                                          int count) {
  if (count < 0) {
...
absl::StatusOr<int> ContributorsToGroups::GetCount(size_t group_index) const {
  if (group_index >= counts_.size()) {
    return absl::InvalidArgumentError(absl::StrCat(
        "Group index ", group_index,
        " is out of bounds. Maximum group index is: ", counts_.size() - 1));
  }
  return counts_[group_index];
}
