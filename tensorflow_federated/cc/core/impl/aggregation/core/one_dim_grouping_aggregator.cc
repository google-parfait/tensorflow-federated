...
Status OneDimBaseGroupingAggregator::ValidateInputsHelper(
  ...
  if (ordinals->dtype() != DT_INT64) {
    return TFF_STATUS(INVALID_ARGUMENT)
           << "OneDimBaseGroupingAggregator::ValidateInputsHelper: dtype "
              "mismatch "
  ...
  if (ordinals->shape() != tensor->shape()) {
    return TFF_STATUS(INVALID_ARGUMENT)
           << "OneDimBaseGroupingAggregator::ValidateInputsHelper: tensor "
              "shape "
  ...
  if (num_dimensions > (size_t)1) {
    return TFF_STATUS(INVALID_ARGUMENT)
           << "OneDimBaseGroupingAggregator::ValidateInputsHelper: Only 1 "
              "dimensional tensors supported. Input tensor has "
           << num_dimensions << " dimensions.";
  }
  if (!ordinals->is_dense() || !tensor->is_dense()) {
    return TFF_STATUS(INVALID_ARGUMENT)
           << "OneDimBaseGroupingAggregator::ValidateInputsHelper: Only dense "
