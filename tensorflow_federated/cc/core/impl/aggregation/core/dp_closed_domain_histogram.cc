...
template <typename T>
std::unique_ptr<NumericalMechanism> CopyAggregateFromColumn(
    const Tensor& column_of_aggregates, int64_t ordinal,
    MutableVectorData<T>& container,
    std::unique_ptr<NumericalMechanism> mechanism) {
  T value = (mechanism == nullptr) ? 0 : mechanism->AddNoise(0);
...
// Extract the norm bounds from the i-th aggregation's intrinsic and create a
// noise mechanism for that aggregation. Templated to match the type of the
// output of the aggregation.
template <typename OutputType>
Status CreateIthMechanism(
    int64_t i, double epsilon, double delta, int64_t l0_bound,
  ...
  OutputType linfinity_bound =
      intrinsic.parameters[kLinfinityIndex].CastToScalar<OutputType>();
  ...
  DPHistogramBundle<OutputType> noise_mechanism;

  TFF_ASSIGN_OR_RETURN(noise_mechanism, CreateDPHistogramBundle(
                                            i, epsilon, delta, l0_bound,
                                            linfinity_bound, l1_bound, l2_bound,
                                            /*open_domain=*/false));
  mechanisms.push_back(std::move(noise_mechanism.mechanism));
  laplace_was_used.push_back(noise_mechanism.use_laplace);
  return absl::OkStatus();
}
template <>
Status CreateIthMechanism<string_view>(
    int64_t i, double epsilon, double delta, int64_t l0_bound,
    const Intrinsic& intrinsic,
    std::vector<std::unique_ptr<NumericalMechanism>>& mechanisms,
    std::vector<bool>& laplace_was_used) {
  return TFF_STATUS(UNIMPLEMENTED)
         << "CreateIthMechanism: string_view is not supported.";
}
...
StatusOr<OutputTensorList> DPClosedDomainHistogram::Report() && {
  ...
  for (int i = 0; i < intrinsics().size(); ++i) {
    ...
    DTYPE_CASES(intrinsic.outputs[0].dtype(), OutputType,
                TFF_RETURN_IF_ERROR(CreateIthMechanism<OutputType>(
                    i, epsilon_per_agg_, delta_per_agg_, l0_bound_, intrinsic,
                    mechanisms, laplace_was_used_)));
  }
    ...
    for (int64_t i = 0; i < noisy_aggregate_data.size(); i++) {
      ...
      if (key_to_output < num_keys_per_input()) {
        ...
        NUMERICAL_ONLY_DTYPE_CASES(
            ...
            mechanisms[mech_to_use] = CopyAggregateFromColumn<T>(
                column_of_aggregates, ordinal,
                dynamic_cast<MutableVectorData<T>&>(container),
                std::move(mechanisms[mech_to_use])));
