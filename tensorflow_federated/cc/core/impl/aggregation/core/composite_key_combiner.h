...
//
// CompositeKey is freely copyable and all copies of the key share the same
// interned data which is owned by the CompositeKeyStore and sub-allocated from
...
// they were string_views of a given size.
class CompositeKeyMap final
 ...
 public:
  ...
  //
  // If the composite key is new, the ordinal assigned to it will be
  // current_ordinal, which is then incremented.
  //
  // Returns the ordinal associated with the composite key and a boolean
  // indicating whether the composite key was newly inserted.
  inline std::pair<int64_t, bool> InsertAndGetOrdinal(
      CompositeKey composite_key) {
    auto [it, inserted] = insert({composite_key, current_ordinal_});
    if (inserted) {
...
class CompositeKeyStore final {
 public:
  ...
  explicit CompositeKeyStore(size_t key_size, size_t block_size = 1024)
      : key_size_in_units_(GetKeySizeInUnits(key_size)),
        block_size_in_units_(block_size * key_size_in_units_),
  ...
  inline CompositeKey CurrentKey() const {
    return CompositeKey{reinterpret_cast<char*>(current_block_->data() +
                                                next_key_index_in_units_)};
  }

  // Advances the key to the next available slot, allocating a new block if
  // necessary.
  inline void AdvanceKey() {
    next_key_index_in_units_ += key_size_in_units_;
    if (next_key_index_in_units_ + key_size_in_units_ >
        current_block_->size()) {
      // No space left in the current block for another key.
      // Allocate a new block to get ready for the next key.
      key_storage_.push_back(NextBlock());
      current_block_ = &key_storage_.back();
      next_key_index_in_units_ = 0;
    }
  }

 private:
  // Key blocks are allocated as std::vector<uint32_t>
  using KeyUnit = uint32_t;
  using KeyBlock = std::vector<KeyUnit>;
  static constexpr size_t kKeyUnitSize = sizeof(KeyUnit);

  inline static size_t GetKeySizeInUnits(size_t key_size) {
    TFF_CHECK(key_size % kKeyUnitSize == 0);
    return key_size / kKeyUnitSize;
  }

  inline KeyBlock NextBlock() const {
    return std::vector<KeyUnit>(block_size_in_units_, 0);
  }

  // The size of the composite key in units.
  size_t key_size_in_units_;
  // The size of a block of keys in units.
  size_t block_size_in_units_;
  // Key storage, pre-allocated to hold one block of keys.
  std::vector<KeyBlock> key_storage_;
  KeyBlock* current_block_;
  // Next index in the current block of keys.
  size_t next_key_index_in_units_ = 0;
};
...
class CompositeKeyCombiner {
 ...
 protected:
  ...
  // then insert each composite key into the map and obtain a possibly
  // new ordinal. (refer to the end of this file for the definition of that
  // function)
  //
