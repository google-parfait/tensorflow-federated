...
GroupByAggregator::GroupByAggregator(
  ...
  if (min_contributors_to_group.has_value()) {
    ...
    contributors_to_groups_ = ContributorsToGroups(
        *min_contributors_to_group, std::move(contributor_counts));
  }
}
...
StatusOr<OutputTensorList> GroupByAggregator::Report() && {
  ...
  const std::vector<int>& counts = contributors_to_groups_.GetCounts();
...
StatusOr<std::vector<std::string>> GroupByAggregator::Partition(
  ...
  const std::vector<int>& contributor_counts =
      contributors_to_groups_.GetCounts();
