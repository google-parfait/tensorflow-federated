...
StatusOr<std::vector<std::string>> GroupByAggregator::Serialize(
  ...
  if (num_partitions == 1) {
    ...
    for (auto const& nested_aggregator : aggregators_) {
      auto nested_aggregator_proto = nested_aggregator->ToProto(
          /*num_partitions=*/1, /*hashes=*/{});
      TFF_CHECK(nested_aggregator_proto.size() == 1);
      nested_aggregators_proto->Add(std::move(nested_aggregator_proto[0]));
    }
  ...
  for (auto const& nested_aggregator : aggregators_) {
    auto nested_aggregator_protos =
        nested_aggregator->ToProto(num_partitions, hashes);
    TFF_CHECK(nested_aggregator_protos.size() == num_partitions);
    for (int i = 0; i < num_partitions; ++i) {
      *states[i].mutable_nested_aggregators()->Add() =
          std::move(nested_aggregator_protos[i]);
    }
  }
