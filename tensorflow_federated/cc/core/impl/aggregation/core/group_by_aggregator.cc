...
StatusOr<Tensor> HashKeys(const OutputTensorList& keys, int n) {
  ...
  for (const auto& key : keys) {
    switch (key.dtype()) {
      case tensorflow_federated::aggregation::DT_STRING: {
        auto string_values = key.AsAggVector<std::string_view>();
        for (const auto& [index, value] : string_values) {
          hashes[index] = CombineHashes(hashes[index], absl::HashOf(value));
        }
        break;
      }
      case tensorflow_federated::aggregation::DT_INT64: {
        auto int64_values = key.AsAggVector<int64_t>();
        for (const auto& [index, value] : int64_values) {
          hashes[index] = CombineHashes(hashes[index], absl::HashOf(value));
        }
        break;
      }
      case tensorflow_federated::aggregation::DT_INT32: {
        auto int32_values = key.AsAggVector<int32_t>();
        for (const auto& [index, value] : int32_values) {
          hashes[index] = CombineHashes(hashes[index], absl::HashOf(value));
        }
        break;
      }
      case tensorflow_federated::aggregation::DT_DOUBLE: {
        auto double_values = key.AsAggVector<double>();
        for (const auto& [index, value] : double_values) {
          hashes[index] = CombineHashes(hashes[index], absl::HashOf(value));
        }
        break;
      }
      case tensorflow_federated::aggregation::DT_FLOAT: {
        auto float_values = key.AsAggVector<float>();
        for (const auto& [index, value] : float_values) {
          hashes[index] = CombineHashes(hashes[index], absl::HashOf(value));
        }
        break;
      }
      default:
        return TFF_STATUS(INVALID_ARGUMENT)
               << "Unsupported tensor dtype: " << key.dtype();
    }
  }
  for (int i = 0; i < size; ++i) {
    hashes[i] = hashes[i] % n;
  }
  auto output_tensor_data = std::make_unique<MutableVectorData<int64_t>>(
      hashes.begin(), hashes.end());
  return Tensor::Create(tensorflow_federated::aggregation::DT_INT64,
