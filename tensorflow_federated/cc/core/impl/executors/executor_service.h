...
class ExecutorService : public v0::ExecutorGroup::Service {
 ...
 public:
  ...
  explicit ExecutorService(const ExecutorFactory& executor_factory)
      : executor_resolver_(ExecutorResolver(executor_factory)) {}
  ...
  class ExecutorResolver {
   public:
    ...
    absl::StatusOr<ExecutorEntry> ExecutorForId(const ExecutorId&,
                                                absl::string_view);
    // Returns an executor with the specified requirements. May construct a new
    // executor; only returns a no-OK status if this construction fails or
    // another internal error occurs.
    absl::StatusOr<ExecutorEntry> ExecutorForRequirements(
