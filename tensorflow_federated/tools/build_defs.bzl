...
def if_static(extra, framework_shared_object = []):
    """Include specific extra deps or srcs when building statically.

    Args:
      extra: A list of deps or srcs to always include.
      framework_shared_object: A list of deps or srcs to include only when building dynamically
        in OSS.

    Returns:
      A dictionary containing the dependencies.
    """
    return_value = {
        "//conditions:default": extra,
    }
    if tff_is_oss:
        shared_object_label = str(Label("@org_tensorflow//tensorflow:framework_shared_object"))
        return_value[shared_object_label] = framework_shared_object
