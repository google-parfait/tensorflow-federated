...
def _update_dir_method(module: _ModuleType,
                       seen_modules: set[_ModuleType]) -> None:
  ...
  tff_submodules = tuple(
      ...
      if _inspect.ismodule(a) and 'tensorflow_federated' in getattr(
          a, '__file__', '') and a not in seen_modules)
  ...
  _logging.info('Module %s had imported symbols %s', module, imported_symbols)
