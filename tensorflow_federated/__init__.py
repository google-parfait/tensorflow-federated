...
def _update_dir_method(module: _ModuleType,
  ...
  def _is_tff_submodule(attribute):
    return (attribute is not None and _inspect.ismodule(attribute) and
            'tensorflow_federated' in getattr(attribute, '__file__', ''))
  ...
  imported_symbols = [
      symbol_name for symbol_name in imported_symbols
