...
def _build_fed_eval_client_work(
    ...
    metric_aggregation_process: aggregation_process.AggregationProcess,
  ...
  def init_fn():
    return metric_aggregation_process.initialize()
  ...
  def next_fn(state, model_weights, client_data):
    ...
    metrics_output = metric_aggregation_process.next(
...
def build_fed_eval(
  ...
  *   `set_model_weights`: A `tff.Computation` with type signature
  ...
  client_work = _build_fed_eval_client_work(model_fn,
                                            metrics_aggregation_process,
                                            batch_type, model_weights_type,
                                            use_experimental_simulation_loop)

  # The `client_work` returns empty model update and update weight.
  client_work_result_type = client_work.next.type_signature.result
  model_update_type = client_work_result_type.result.member.update
  model_update_weight_type = client_work_result_type.result.member.update_weight
