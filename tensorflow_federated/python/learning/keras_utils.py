  ...
  if metric_constructors is None:
    ...
    for metric_constructor in metric_constructors:
      if not callable(metric_constructor):
        raise TypeError(f'The {metric_constructor} is not callable.')
    ...
    if not metrics and metric_constructors:
      for metric_constructor in metric_constructors:
        ...
        if not isinstance(metric, tf.keras.metrics.Metric):
          raise TypeError(f'{metric} is not a no-arg callable that creates a '
                          '`tf.keras.metrics.Metric`.')
        self._metrics.append(metric_constructor())
