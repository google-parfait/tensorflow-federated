...
class ModelWeights(NamedTuple):
  ...
  def from_model(cls, model):
    ...
    if (
      ...
      return cls(
          [_unwrap_variable(v) for v in model.trainable_variables],
          [_unwrap_variable(v) for v in model.non_trainable_variables],
      )
    else:
  ...
  def from_tff_result(cls, struct):
    if not isinstance(struct, structure.Struct):
      struct = structure._from_container(struct, recursive=True)  # pylint: disable=protected-access
    return cls(
