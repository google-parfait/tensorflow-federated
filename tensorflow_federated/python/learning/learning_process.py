...
class LearningProcessOutput:
  ...
  Attributes:
    ...
    metrics: Metrics derived from the applying the process to a given state and
      input. This is intended for surfacing values to track the progress of a
      learning process, and is not directly intended to be used as input to
      `LearningProcess.next`.
  """
  state = attr.ib()
  metrics = attr.ib()
...
class LearningProcess(iterative_process.IterativeProcess):
  ...
  Both `initialize` and `next` must be `tff.Computation`s, each with the
  ...
  denoting the type of a single member of that sequence.
  """
  ...
  def __init__(self, initialize_fn: computation_base.Computation,
    ...
    Args:
     ...
     report_fn: A `tff.Computation` that accepts an input `S` where the output
       ...
       on held-out data. The report function need not be invertible. That is, a
       state may not be uniquely determined by `report_fn(state)`.
