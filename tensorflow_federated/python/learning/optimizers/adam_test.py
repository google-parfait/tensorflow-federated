...
class AdamTest(optimizer_test_utils.TestCase, parameterized.TestCase):
  ...
  def test_state_structure(self):
    ...
    self.assertIn(adam.LEARNING_RATE_KEY, state)
    self.assertIn(adam.BETA_1_KEY, state)
    self.assertIn(adam.BETA_2_KEY, state)
    self.assertIn(adam.EPSILON_KEY, state)
    self.assertIn(adam.STEP_KEY, state)
    self.assertIn(adam.PRECONDITIONER_KEY, state)
    self.assertIn(adam.ACCUMULATOR_KEY, state)
