...
class KerasUtilsTest(tf.test.TestCase):
  ...
  def test_from_keras_model_properties(self):
    ...
    self.assertEqual(
        recon_model.global_trainable_variables, keras_model.trainable_variables
    )
    self.assertEqual(
  ...
  def test_from_keras_model_local_layers_properties(self):
    ...
    self.assertEqual(
        recon_model.global_trainable_variables,
    ...
    self.assertEqual(
        recon_model.local_trainable_variables,
        keras_model.trainable_variables[-2:],
    )
    self.assertEmpty(recon_model.local_non_trainable_variables)
    self.assertEqual(input_spec, recon_model.input_spec)
  ...
  def test_from_keras_model_forward_pass(self):
    ...
    self.assertAllEqual(
        batch_output.labels, tf.zeros(shape=[10, 1], dtype=tf.int32)
    )
    ...
    self.assertAllEqual(
        batch_output.labels, tf.ones(shape=[5, 1], dtype=tf.int32)
    )
