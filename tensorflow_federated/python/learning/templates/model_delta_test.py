...
from tensorflow_federated.python.learning.templates import model_delta
...
class ModelDeltaClientWorkComputationTest(test_case.TestCase,
  ...
  def test_type_properties(self, optimizer, weighting):
    ...
    client_work_process = model_delta.build_model_delta_client_work(
  ...
  def test_created_keras_optimizer_raises(self):
    with self.assertRaises(TypeError):
      model_delta.build_model_delta_client_work(
...
class ModelDeltaClientWorkExecutionTest(test_case.TestCase,
  ...
  def test_keras_tff_client_work_equal(self, weighting):
    ...
    client_update_keras = model_delta.build_model_delta_with_keras_optimizer(
        model_fn=self.create_model, weighting=weighting)
    client_update_tff = model_delta.build_model_delta_with_tff_optimizer(
        model_fn=self.create_model, weighting=weighting)
  ...
  def test_client_tf(self, simulation, optimizer_kwargs, expected_norm,
                     weighting):
    client_tf = model_delta.build_model_delta_with_keras_optimizer(
  ...
  def test_proximal_term_yields_smaller_model_delta(self, optimizer):
    simple_process = model_delta.build_model_delta_client_work(
        self.create_model,
        optimizer,
        client_weighting=client_weight_lib.ClientWeighting.NUM_EXAMPLES,
        proximal_strength=0.0)
    proximal_process = model_delta.build_model_delta_client_work(
        self.create_model,
        sgdm.build_sgdm(1.0),
        client_weighting=client_weight_lib.ClientWeighting.NUM_EXAMPLES,
