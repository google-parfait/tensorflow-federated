...
def build_model_delta_with_tff_optimizer(
    model_fn,
    weighting,
  ...
  Args:
    ...
    proximal_strength: A nonnegative float, L2 regularization strength of the
      model delta.
  ...
  def client_update(optimizer, initial_weights, data):
    ...
    def reduce_fn(state, batch):
      ...
      proximal_term = tf.nest.map_structure(
          lambda x, y: proximal_strength * (y - x), model_weights.trainable,
    ...
    def initial_state_for_reduce_fn():
      ...
      trainable_tensor_specs = tf.nest.map_structure(
          ...
          _flat_tuple(model_weights.trainable))
...
# TODO(b/213433744): Make this method private.
def build_model_delta_with_keras_optimizer(
    model_fn,
    weighting,
    proximal_strength=0.0,
    use_experimental_simulation_loop: bool = False):
...
def build_model_delta_client_work(
  ...
  Args:
    ...
    proximal_strength: A nonnegative float representing the parameter of the
  ...
  if isinstance(optimizer, optimizer_base.Optimizer):
    ...
    def client_update_computation(initial_model_weights, dataset):
      client_update = build_model_delta_with_tff_optimizer(
          model_fn, client_weighting, proximal_strength,
          use_experimental_simulation_loop)
    ...
    def client_update_computation(initial_model_weights, dataset):
      ...
      client_update = build_model_delta_with_keras_optimizer(
          model_fn, client_weighting, proximal_strength,
          use_experimental_simulation_loop)
