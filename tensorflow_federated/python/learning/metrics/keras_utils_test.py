...
class CreateFunctionalMetricTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def assert_binary_metric_variableless_functions(self, initialize_fn,
                                                  update_fn, finalize_fn):
    self.assert_no_variable_ops(
        initialize_fn.get_concrete_function().graph.as_graph_def())
    state_spec = tf.nest.map_structure(tf.TensorSpec.from_tensor,
                                       initialize_fn())
    self.assert_no_variable_ops(
    ...
    self.assert_no_variable_ops(
        finalize_fn.get_concrete_function(
            state=state_spec).graph.as_graph_def())
  ...
  def test_custom_tff_metrics(self, metric_constructor):
    ...
    initialize, update, finalize = keras_utils.create_functional_metric_fns(
        metric_constructor)
    self.assert_binary_metric_variableless_functions(initialize, update,
                                                     finalize)
    state = initialize()
