...
class CheckMetricFinalizersTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_valid_finalizers_does_not_raise(self):
  ...
  @parameterized.named_parameters(
      ...
      ('non_str_key', collections.OrderedDict([(1.0, _tf_mean)
                                              ]), 'metric_finalizers key'),
      ('non_callable', collections.OrderedDict(mean=_tf_mean([2.0, 1.0])),
...
class CheckUnfinalizedMetricsTypeTest(tf.test.TestCase, parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ('struct_type', computation_types.StructType(
          [(None, tf.int32)]), '`tff.types.StructWithPythonType`'),
      ('ordereddict', collections.OrderedDict(num_example=tf.int32),
       '`tff.types.StructWithPythonType`'),
      ('list_container', type_conversions.type_from_tensors(
          [1.0, 2.0]), 'Python container'))
...
class CheckFinalizersMatchUnfinalizedMetricsTypeTest(tf.test.TestCase,
  ...
  @parameterized.named_parameters(
      ('more_metrics_in_finalizers',
       collections.OrderedDict(
           ...
           mean=_tf_mean), collections.OrderedDict(num_examples=1),
  ...
  def test_not_match_raises(self, metric_finalizers, local_unfianlized_metrics,
                            expected_regex):
