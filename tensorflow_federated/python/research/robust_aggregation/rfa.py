...
def build_stateless_robust_aggregation(model_type, num_communication_passes=5, tolerance=1e-6):
    ...
    Args:
        ...
        num_communication_passes: number of rounds in the smoothed Weiszfeld algorithm (min. 1).
        tolerance: smoothing parameter of smoothed Weiszfeld algorithm. Default 1e-6.
    ...
    if num_communication_passes < 1:
        raise ValueError('(Robust) Aggregation requires at least one communication pass')
    ...
    def update_weight_fn(weight, server_model, client_model):
        ...
        return weight / tf.maximum(tolerance, tf.sqrt(sqnorm))
 ...
 """Builds the TFF computations for robust federated aggregation using the RFA Algorithm.
  Args:
    ...
    num_communication_passes: Number of communication passes for the smoothed
      Weiszfeld algorithm to compute the approximate geometric median. The default
      is 5 and it has to be an interger at least 1.
