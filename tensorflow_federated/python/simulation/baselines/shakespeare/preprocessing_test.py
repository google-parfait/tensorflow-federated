...
from tensorflow_federated.python.simulation.baselines.shakespeare import preprocessing
...
class TokenizeFnTest(tf.test.TestCase):
  ...
  def test_tokenize_fn_returns_expected_elements(self):
    pad, _, bos, eos = preprocessing.get_special_tokens()
    to_tokens = preprocessing._build_tokenize_fn(split_length=5)
    tokens = self.evaluate(to_tokens({'snippets': tf.constant('abc')}))
    self.assertAllEqual(tokens, [bos, 64, 42, 21, eos])
    to_tokens = preprocessing._build_tokenize_fn(split_length=12)
  ...
  def test_tokenize_appends_eos_token(self):
    _, oov, bos, eos = preprocessing.get_special_tokens()
    to_tokens = preprocessing._build_tokenize_fn(split_length=5)
...
class SplitTargetTest(tf.test.TestCase):
  ...
  def test_split_target(self):
    example = self.evaluate(
        preprocessing._split_target(tf.constant([[1, 2, 3]])))
    self.assertAllEqual(([[1, 2]], [[2, 3]]), example)
    example = self.evaluate(
        preprocessing._split_target(tf.constant([[1, 2, 3], [4, 5, 6]])))
    self.assertAllEqual((
        [[1, 2], [4, 5]],
        [[2, 3], [5, 6]],
    ), example)
