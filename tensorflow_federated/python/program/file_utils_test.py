...
class ReadSavedModelTest(parameterized.TestCase):
  ...
  def test_returns_value_with_path_str(self):
    module = file_utils._ValueModule(1)
    path = self.create_tempdir()
    path = path.full_path
    tf.saved_model.save(module, path, signatures={})

    actual_value = file_utils.read_saved_model(path)

    self.assertEqual(actual_value, 1)

  def test_returns_value_with_path_path_like(self):
    module = file_utils._ValueModule(1)
    path = self.create_tempdir()
...
class WriteSavedModelTest(parameterized.TestCase):
  ...
  def test_writes_to_new_file_with_path_str(self):
    path = self.create_tempdir()
    path = path.full_path
    shutil.rmtree(path)
    self.assertFalse(os.path.exists(path))

    file_utils.write_saved_model(1, path)
  ...
  def test_writes_to_new_file_with_path_path_like(self):
    path = self.create_tempdir()
    shutil.rmtree(path)
    self.assertFalse(os.path.exists(path))
  ...
  def test_writes_to_existing_file(self):
    ...
    file_utils.write_saved_model(1, path, True)
  ...
  def test_raises_type_error_with_overwrite(self, overwrite):
    ...
    with self.assertRaises(TypeError):
      file_utils.write_saved_model(1, path, overwrite)
