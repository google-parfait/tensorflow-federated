...
from tensorflow_federated.python.program import platform
...
class NumpyValueReferenceTest(parameterized.TestCase):
  ...
  def test_init_raises_type_error_with_type_signature(self, type_signature):
    with self.assertRaises(TypeError):
      platform.NumpyValueReference(value=1, type_signature=type_signature)
  ...
  def test_get_value_returns_value(self, value, type_signature, expected_value):
    value_reference = platform.NumpyValueReference(
...
class ContainsOnlyServerPlacedDataTest(parameterized.TestCase):
  ...
  def test_returns_true(self, type_signature):
    result = platform._contains_only_server_placed_data(type_signature)
  ...
  def test_raises_type_error_with_type_signature(self, type_signature):
    with self.assertRaises(TypeError):
      platform._contains_only_server_placed_data(type_signature)
...
class CreateStructureOfValueReferencesTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ...
      ('struct_unnamed',
       ...
       structure.Struct([
           (None, platform.NumpyValueReference(
      ...
      ('struct_named',
       ...
       structure.Struct([
           ('a', platform.NumpyValueReference(
               True, computation_types.TensorType(tf.bool))),
           ('b', platform.NumpyValueReference(
               1, computation_types.TensorType(tf.int32))),
           ('c', platform.NumpyValueReference(
               'a', computation_types.TensorType(tf.string))),
       ])),
      ('struct_nested',
  ...
  def test_returns_value(self, value, type_signature, expected_value):
    actual_value = platform._create_structure_of_value_references(
        value=value, type_signature=type_signature)
  ...
  def test_raises_type_error_with_type_signature(self, type_signature):
    with self.assertRaises(TypeError):
      platform._create_structure_of_value_references(
...
class MaterializeStructureOfValueReferencesTest(parameterized.TestCase):
  ...
  # pyformat: enable
  def test_returns_value(self, value, type_signature, expected_value):
    actual_value = platform._materialize_structure_of_value_references(
        value=value, type_signature=type_signature)
  ...
  def test_raises_type_error_with_type_signature(self, type_signature):
    with self.assertRaises(TypeError):
      platform._materialize_structure_of_value_references(
          value=1, type_signature=type_signature)
...
class NativeFederatedContextTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ('local_cpp_async',
       cpp_execution_contexts.create_local_async_cpp_execution_context()),
      ('local_cpp_sync',
       cpp_execution_contexts.create_local_cpp_execution_context()),
  )
  def test_init_does_not_raise_type_error_with_context(self, context):
    try:
      platform.NativeFederatedContext(context)
    except TypeError:
      self.fail('Raised TypeError unexpectedly.')
  ...
  # pyformat: enable
  def test_ingest_returns_result(self, value, type_signature, expected_value):
    context = cpp_execution_contexts.create_local_cpp_execution_context()
    context = platform.NativeFederatedContext(context)
  ...
  @parameterized.named_parameters(
      ('local_cpp_sync',
       cpp_execution_contexts.create_local_cpp_execution_context()),)
  def test_invoke_returns_result(self, context):
    context = platform.NativeFederatedContext(context)
  ...
  def test_invoke_does_not_raise_value_error_with_comp(self):
    ...
    try:
      with mock.patch.object(
          platform, '_contains_only_server_placed_data', return_value=True):
  ...
  def test_invoke_raises_value_error_with_comp(self):
    ...
    with self.assertRaises(ValueError):
      with mock.patch.object(
          platform, '_contains_only_server_placed_data', return_value=False):
        context.invoke(return_one, None)

  @parameterized.named_parameters(
      ('local_cpp_sync',
       cpp_execution_contexts.create_local_cpp_execution_context()),)
  def test_computation_returns_result(self, context):
    context = platform.NativeFederatedContext(context)
...
class DatasetDataSourceIteratorTest(parameterized.TestCase, tf.test.TestCase):
  ...
  def test_init_does_not_raise_type_error(self):
    ...
    try:
      platform.DatasetDataSourceIterator(
  ...
  def test_select_returns_data(self, number_of_clients):
    ...
    federated_type = computation_types.FederatedType(
        computation_types.SequenceType(tf.int32), placements.CLIENTS)
    iterator = platform.DatasetDataSourceIterator(
        datasets=datasets, federated_type=federated_type)
...
class DatasetDataSourceTest(parameterized.TestCase):
  ...
  def test_init_sets_federated_type(self, tensors, dtype):
    ...
    data_source = platform.DatasetDataSource(datasets=datasets)
  ...
  def test_init_raises_type_error(self, datasets):
    with self.assertRaises(TypeError):
      platform.DatasetDataSource(datasets=datasets)
