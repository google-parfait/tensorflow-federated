...
def _read_from_csv(
    file_path: os.PathLike) -> Tuple[Set[str], List[Dict[str, Any]]]:
...
def _write_to_csv(file_path: os.PathLike, fieldnames: Iterable[str],
...
class CSVFileReleaseManagerReadValuesFromCSVTest(absltest.TestCase):
  ...
  def test_returns_values_from_empty_file(self):
    ...
    fieldnames, values = file_release_mngr._read_values_from_csv()
...
class CSVFileReleaseManagerWriteValuesToCSVTest(parameterized.TestCase):
  ...
  def test_writes_values_to_empty_file(self, fieldnames, values):
    ...
    file_release_mngr._write_values_to_csv(fieldnames=fieldnames, values=values)

    actual_fieldnames, actual_values = _read_from_csv(temp_file)
    expected_fieldnames = set(fieldnames)
...
class CSVFileReleaseManagerWriteValueToCSVTest(parameterized.TestCase):
  ...
  def test_writes_value_to_empty_file(self, value):
    ...
    file_release_mngr._write_value_to_csv(value)

    actual_fieldnames, actual_values = _read_from_csv(temp_file)
    expected_fieldnames = set([file_release_manager._KEY_FIELDNAME])
    expected_fieldnames.update(value.keys())
    self.assertEqual(actual_fieldnames, expected_fieldnames)
  ...
  def test_write_value_to_existing_file(self, value):
    ...
    actual_fieldnames, actual_values = _read_from_csv(temp_file)
    expected_fieldnames = set([file_release_manager._KEY_FIELDNAME, 'a', 'b'])
    expected_fieldnames.update(value.keys())
    self.assertEqual(actual_fieldnames, expected_fieldnames)
...
class CSVFileReleaseManagerAppendValueToCSVTest(parameterized.TestCase):
  ...
  def test_appends_value_to_empty_file(self, value):
    ...
    file_release_mngr._append_value_to_csv(value)
  ...
  def test_raises_permission_denied_error(self):
    ...
    with mock.patch('csv.DictWriter.writerow') as mock_writerow:
      ...
      with self.assertRaises(
        ...
        file_release_mngr._append_value_to_csv({})
...
class CSVFileReleaseManagerReleaseTest(parameterized.TestCase):
  ...
  # pyformat: enable
  def test_calls_append_value_to_csv(self, value, key):
    temp_file = self.create_tempfile()
    os.remove(temp_file)
    file_release_mngr = file_release_manager.CSVFileReleaseManager(
        file_path=temp_file, save_mode=file_release_manager.SaveMode.APPEND)
    ...
    with mock.patch.object(file_release_mngr,
                           '_append_value_to_csv') as mock_append_value_to_csv:
      file_release_mngr.release(value, key)

      mock_append_value_to_csv.assert_called_once()
      call = mock_append_value_to_csv.mock_calls[0]
      _, args, _ = call
      actual_value, = args
      expected_fieldnames = set([file_release_manager._KEY_FIELDNAME])
      expected_fieldnames.update(value.keys())
  ...
  # pyformat: enable
  def test_calls_write_value_to_csv(self, value, key):
    temp_file = self.create_tempfile()
    os.remove(temp_file)
    file_release_mngr = file_release_manager.CSVFileReleaseManager(
        file_path=temp_file, save_mode=file_release_manager.SaveMode.WRITE)
    ...
    with mock.patch.object(file_release_mngr,
                           '_write_value_to_csv') as mock_write_value_to_csv:
      file_release_mngr.release(value, key)

      mock_write_value_to_csv.assert_called_once()
      call = mock_write_value_to_csv.mock_calls[0]
      _, args, _ = call
      actual_value, = args
      expected_fieldnames = set([file_release_manager._KEY_FIELDNAME])
      expected_fieldnames.update(value.keys())
