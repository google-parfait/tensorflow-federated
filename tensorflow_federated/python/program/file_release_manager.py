...
import os.path
import random
from typing import Any, Dict, Iterable, List, Mapping, Tuple, Set, Union
...
class CSVFileReleaseManager(release_manager.ReleaseManager):
  ...
  def __init__(self,
    ...
    if tf.io.gfile.exists(self._file_path):
      fieldnames, values = self._read_values_from_csv()
      ...
      self._write_values_to_csv([_KEY_FIELDNAME], [])
      self._latest_key = None

  def _read_values_from_csv(self) -> Tuple[Set[str], List[Dict[str, Any]]]:
  ...
  def _write_values_to_csv(self, fieldnames: Iterable[str],
  ...
  def _write_value_to_csv(self, value: Mapping[str, Any]):
    """Writes `value` to the managed CSV."""
    fieldnames, values = self._read_values_from_csv()
    fieldnames.update(value.keys())
    values.append(value)
    self._write_values_to_csv(fieldnames, values)

  def _append_value_to_csv(self, value: Mapping[str, Any]):
    """Appends `value` to the managed CSV."""
    with tf.io.gfile.GFile(self._file_path, 'a+') as file:
      reader = csv.DictReader(file, quoting=csv.QUOTE_NONNUMERIC)
      new_fieldnames = set(value.keys())
      if new_fieldnames.issubset(reader.fieldnames):
        writer = csv.DictWriter(
        ...
        self._write_value_to_csv(value)
  ...
  def _remove_values_after(self, key: int):
    ...
    filtered_values = []
    filtered_fieldnames = set([_KEY_FIELDNAME])
    _, values = self._read_values_from_csv()
    for value in values:
      if _KEY_FIELDNAME in value and value[_KEY_FIELDNAME] <= key:
        filtered_values.append(value)
        filtered_fieldnames.update(value.keys())
    self._write_values_to_csv(filtered_fieldnames, filtered_values)
  ...
  def release(self, value: Any, key: int):
    ...
    if self._save_mode == SaveMode.APPEND:
      self._append_value_to_csv(flattened_value)
    elif self._save_mode == SaveMode.WRITE:
      self._write_value_to_csv(flattened_value)
    self._latest_key = key
