...
# Supported flattening operations.
FLATTENERS = [
...
class DistributedDpSumFactory(factory.UnweightedAggregationFactory):
  ...
  distributed discrete Gaussian mechanism (go/ddp-fl) and the distributed
  Skellam mechanism (go/ddp-fl-skellam).
  ...
  def __init__(self,
    ...
    Args:
      ...
      beta: (Optional) The conditional randomized rounding bias. Must be a float
        ...
        sensitivity inflation. Defaults to exp(-0.5).
      mechanism: (Optional) The distributed DP mechanism to use. Possible
        ...
        the default).
      flattener: (Optional) The flattening operation used to spread out input
      ...
      auto_l2_clip: (Optional) A bool indicating whether to adaptively adjust
        the L2 norm clipping after each round. Note that this involves private
        quantile estimation which would result in a larger effective
        `noise_multiplier` for the actual client values. The algorithm used is
        based on https://arxiv.org/pdf/1905.03871.pdf with the geometric update
    ...
    _check_str(flattener, 'flattener', FLATTENERS)
  ...
  def _build_aggregation_factory(self):
    ...
    if self._mechanism == 'distributed_dgauss':
      ...
      dp_query = distributed_skellam_query.DistributedSkellamSumQuery(
    ...
    nested_factory = robust.clipping_factory(
        clipping_norm=self._l2_clip, inner_agg_factory=nested_factory)
  ...
  def _unpack_measurements(self, agg_measurements):
    ...
    l2_clip_metrics = rotate_metrics[self._flattener]
  ...
  def _derive_measurements(self, agg_state, agg_measurements):
    ...
    actual_num_clients = intrinsics.federated_sum(
    ...
    measurements = collections.OrderedDict(
        ...
        scale_factor=_make_server_metric(discrete_state['scale_factor']),
        scaled_inflated_l2=_make_server_metric(dp_query_state.l2_norm_bound),
        scaled_local_stddev=_make_server_metric(dp_query_state.local_stddev),
        actual_num_clients=actual_num_clients,
        padded_dim=_make_server_metric(padded_dim),
...
def _make_server_metric(server_metric):
  """NOTE(b/163880757): Temporary work-around for server-only metrics.

  The TF-based aggregated ("TF Loader") expects all data needed for cross-round
  aggregation to flow through Federated Aggrtegation, because it extracts the
  partial sums ("accumulators") off the result of Merge on CanonicalForm.
  However, server-only metrics does not pass through Federated Aggregation at
  all, which means the collected metrics may not be correct. We can work around
  this issue by using another trigger value that forces the forces data to
  pass through aggregation path.

  Args:
    server_metric: A scalar metric value placed at tff.SERVER.

  Returns:
    The same server_metric.
  """
  federated_type = typing.cast(computation_types.FederatedType,
                               server_metric.type_signature)
  tensor_type = typing.cast(computation_types.TensorType, federated_type.member)
  np_dtype = tensor_type.dtype.as_numpy_dtype
  trigger = intrinsics.federated_sum(
      intrinsics.federated_value(np_dtype(0), placements.CLIENTS))
  return intrinsics.federated_map(
      computations.tf_computation(lambda x, y: x + y), (trigger, server_metric))
