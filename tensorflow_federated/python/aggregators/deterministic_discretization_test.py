...
class DeterministicDiscretizationExecutionTest(tf.test.TestCase,
                                               parameterized.TestCase):
  ...
  def test_discretize_impl(self, value_type, client_values, expected_sum):
    ...
    expected_measurements = collections.OrderedDict(
        deterministic_discretization=expected_quantized_result, distortion=0.)
  ...
  def test_output_dtype(self, dtype):
    ...
    encoded_x = deterministic_discretization._discretize_struct(
        x, step_size=0.1)
    self.assertEqual(encoded_x.dtype,
                     deterministic_discretization.OUTPUT_TF_TYPE)
