...
class EliasGammaEncodedSumFactory(factory.UnweightedAggregationFactory):
  ...
  def create(self, value_type):
    if not (value_type.is_tensor() and value_type.dtype == tf.int32):
      raise ValueError("Expect value_type to be an int32 tensor, "
                       f"found {value_type}.")
    ...
    def next_fn(state, value):
      ...
      num_elements = intrinsics.federated_value(
          computations.tf_computation(lambda x: tf.cast(x, tf.float64))(
              value_type.shape.num_elements()), placements.SERVER)
      avg_bitrate = intrinsics.federated_map(
          computations.tf_computation(
              lambda x, y: tf.math.divide_no_nan(x=x, y=y)),
