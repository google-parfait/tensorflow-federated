...
class ExecutionTracingProvider(tracing.TracingProvider):
  ...
  def _prefix(self, ctx, type_string):
    ...
    if ctx is None:
      return template.format(indentation=indentation, type=type_string, hash='NO_ID')
    return template.format(indentation=indentation, type=type_string, hash=hash(ctx))
  ...
  def register_executors(self):
    for type_string in [
      ...
      self._register_formatting_strategy(
          type_string, lambda x: '<{} @{}>'.format(type_string, id(x)))
  ...
  def register_computation_types(self):
    for type_string in [
      ...
      self._register_formatting_strategy(
            type_string, lambda x: '<{}>'.format(x))

  def register_primitives(self):
    self._register_formatting_strategy(
        float, lambda x: '<{} : float>'.format(x))
    ...
    self._register_formatting_strategy(
        tuple,
        lambda x: '<' + ', '.join(tuple(
            self._format_object(xi) for xi in x)) + '>')

    self._register_formatting_strategy(
        'EagerTensor',
        lambda x: '<{} @{}>'.format('EagerTensor', id(x)))
