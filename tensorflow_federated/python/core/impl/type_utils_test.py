...
class TypeUtilsTest(test.TestCase, parameterized.TestCase):
  ...
  def test_type_to_tf_tensor_specs_with_invalid_type(self):
    with self.assertRaises(TypeError):
      type_utils.type_to_tf_tensor_specs(tf.constant([0.0]))

  def test_type_to_tf_tensor_specs_with_unnamed_element(self):
    tensor_specs = type_utils.type_to_tf_tensor_specs(
...
class TransformTypePostorderTest(tf.test.TestCase):
  ...
  def test_raises_on_none_type(self):
    with self.assertRaises(TypeError):
      type_utils.transform_type_postorder(None, lambda x: x)

  def test_raises_on_none_function(self):
    with self.assertRaises(TypeError):
      type_utils.transform_type_postorder(
          computation_types.to_type(tf.int32), None)

  def test_raises_on_non_type_first_arg(self):
    with self.assertRaises(TypeError):
      type_utils.transform_type_postorder(tf.int32, None)
  ...
  def test_transforms_tensor(self):
    ...
    result_type, mutated = type_utils.transform_type_postorder(
        orig_type, _convert_tensor_to_float)
    noop_type, not_mutated = type_utils.transform_type_postorder(
  ...
  def test_updates_mutated_bit_at_federated(self):
    orig_type = computation_types.FederatedType(tf.int32, placements.CLIENTS)
    _, mutated = type_utils.transform_type_postorder(
  ...
  def test_updates_mutated_bit_at_tuple(self):
    orig_type = computation_types.to_type([tf.int32, tf.float64])
    _, mutated = type_utils.transform_type_postorder(orig_type,
                                                     _convert_tuple_to_tensor)
    self.assertTrue(mutated)
