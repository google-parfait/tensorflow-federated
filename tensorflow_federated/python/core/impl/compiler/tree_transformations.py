...
def group_block_locals_by_dependency(
  ...
  bindings; that is, they bind variables sequantially, and not in parallel.

  This poses a potential problem in writing an execution stack; we must compute
  the values bound to block's symbols in sequence for semantic correctness,
  even if there is no "true" dependency between these values.

  This transformation groups all block locals by their dependency structure, so
  that every computation bound at each symbol appears after only the
  computations on which it truly depends.

  Note that this is not necessarily the ideal concurrency pattern--this
  transformation is intended to introduce *an* invariant, not necessarily the
  optimal one.

  The paradigmatic use of this transformation is immediately after the
  flattening the body of a computation into single large block as in
  `remove_duplicate_building_blocks` above.

  Args:
    comp: Instance of `building_blocks.ComputationBuildingBlock` to transform.

  Returns:
    A two-tuple, whose first element is a transformed version of `comp`, and
    whose second element is a boolean indicating whether or not any changes were
    made to `comp`.


  Raises:
    TypeError: if `comp` is not a `building_blocks.ComputationBuildingBlock`.
    ValueError: if `comp` violates the assumptions of
      `GroupBlockLocalsByDependency`.
  """
  return _apply_transforms(comp, GroupBlockLocalsByDependency(comp))


RepackedBlockLocalsType = List[List[Tuple[
    str, building_blocks.ComputationBuildingBlock]]]


class GroupBlockLocalsByDependency(transformation_utils.TransformSpec):
  """Groups block-locals statements by dependence.

  For each block under its arg, this transformation constructs an
  equivalent block with a locals sequence which is as short as possible. That is
  to say, computations are grouped by the references they leave unbound, and
  these groups are rebound to new variables in the resulting block. This
  transform is idempotent.

  Example:

  Let x=a,y=b,u=x,v=y,z=u in u

  Would be transformed into:

  Let var1=<a,b>,var2=<var1[0],var1[1]>,var3=var2[0] in var2[0]

  Where var1 represents what could formerly have been referred to as <x, y>,
  var2 represents <u, v> and var3 represents z.
  """
  ...
  def _create_groups_of_block_locals(
      self, block: building_blocks.Block) -> RepackedBlockLocalsType:
    """Packs block locals into groups of tuples whose elements are independent."""
    unbound_reference_map = transformation_utils.get_map_of_unbound_references(
        block)
    captured_symbols = unbound_reference_map[block]
    tuple_groups = []
    bindings_remaining = block.locals

    def _populate_next_tuple_group(
        symbols_remaining: Sequence[Tuple[str, building_blocks.Block]],
        captured_symbols: Set[str],
        previous_tuple_groups: RepackedBlockLocalsType):
      """Packs bindings which refer only to `captured_symbols`.

      This function identifies all the bindings in `symbols_remaining` which
      depend only on symbols in `captured_symbols`, constructs a list of these
      bindings, and appends this list to `tuple_groups`.

      Suppose `symbols_remaining` is nonempty, and the union of
      `symbols_remaining` and `captured_symbols` contains all the
      symbols which are referenced in the bodies of these bindings. This
      condition will be respected by this function on its outputs,
      `leftover_symbols` and `captured_symbols`. Further, `leftover_symbols`
      will contain at most `len(symbols_remaining) - 1` symbols.

      This claim can be verified by noting that a symbol being placed in
      `leftover_symbols` is equivalent to the possibility of forming a
      topological sorting of `symbols_remaining` with this symbol at the root.

      Args:
        symbols_remaining: List of tuples of strings and building blocks,
          representing block locals yet to be packed into tuples.
        captured_symbols: A set of strings representing the union of symbols
          which are unbound in the entire block and those which have already
          been packed into tuples.
        previous_tuple_groups: The groups of block locals already packed into
          tuples.

      Returns:
        A two-tuple whose first element represents those symbols from
        `symbols_remaining` which reference symbols not found in
        `captured_symbols`, and whose second element represents an updated
        set of `captured_symbols`.
      """
      bindings_to_move = []
      leftover_bindings = []
      for name, comp in symbols_remaining:
        unbound_refs_in_comp = unbound_reference_map[comp]
        uncaptured_and_unbound = unbound_refs_in_comp.difference(
            captured_symbols)
        if not uncaptured_and_unbound:
          bindings_to_move.append((name, comp))
        else:
          leftover_bindings.append((name, comp))
      captured_symbols = captured_symbols.union(
          name for name, _ in bindings_to_move)
      next_tuple_groups = previous_tuple_groups + [bindings_to_move]
      return leftover_bindings, captured_symbols, next_tuple_groups

    while bindings_remaining:
      bindings_remaining, captured_symbols, tuple_groups = _populate_next_tuple_group(
          bindings_remaining, captured_symbols, tuple_groups)
    return tuple_groups
  ...
  def _update_old_symbols_to_new_comp(
    ...
    if isinstance(new_comp, building_blocks.Struct):
      ...
      old_symbol, comp = new_comp
      symbols_to_comp_map[old_symbol] = building_blocks.Reference(
          new_symbol, comp.type_signature)

  def transform(
      self, block_to_transform: building_blocks.ComputationBuildingBlock
  ) -> building_blocks.ComputationBuildingBlock:
    """Performs transform specified in class docstring.

    Args:
      block_to_transform: Instance of `building_blocks.ComputationBuildingBlock`
        to transform. If this is a `building_blocks.Block`, will be transformed
        into a block whose locals are of minimal length with respect to a simple
        analysis of dependence.

    Returns:
      A transformed `building_blocks.ComputationBuildingBlock` representing
      identical semantics as `block_to_transform`.

    Raises:
      In the case that `block_to_transform` does not have unique names.
    """
    ...
    for comp_class in locals_partitioned:
      if len(comp_class) == 1:
        # We branch here to prevent unnecessarily wrapping singletons in tuples.
        # This is necessary for idempotency, a highly desirable property here.
    ...
    transformed_new_symbols = []
    modified_list = []
    for name, comp in _strip_old_names_from_new_symbols(new_symbols):
      transformed_comp, modified = transformation_utils.transform_postorder(
          comp, _transform)
      modified_list.append(modified)
      transformed_new_symbols.append((name, transformed_comp))
    transformed_result, modified = transformation_utils.transform_postorder(
        block_to_transform.result, _transform)
    any_modified = any(modified_list) or modified

    return building_blocks.Block(transformed_new_symbols,
                                 transformed_result), any_modified
