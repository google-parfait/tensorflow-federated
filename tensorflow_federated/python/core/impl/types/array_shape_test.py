...
class ArrayShapeTest(parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      ('fully_defined', (2, 3), False),
      ('partially_defined', (1, None), False),
      ('unknown', None, False),
      ('scalar', (), True),
  )
  def test_is_shape_scalar(self, shape, expected_result):
    actual_result = array_shape.is_shape_scalar(shape)
    self.assertEqual(actual_result, expected_result)
  ...
  def test_num_elements_in_shape(self, shape, expected_result):
