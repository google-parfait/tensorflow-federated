...
"""A strategy for resolving federated types and intrinsics in disjoint scopes.
  ...
  * Delegates resolving unplaced types, computations, and processing back to the
    `FederatingExecutor`.
  * Delegates resolving server-side processing to the `server_executor`.
  * Delegates resolving federating types and intrinsics to the target
    executor(s).
"""
...
class ComposingFederatingStrategyValue(executor_value_base.ExecutorValue):
  """A value embedded in a `FederatedExecutor`."""

  def __init__(self, value, type_signature):
    """Creates a `ComposingFederatingStrategyValue` embedding the given `value`.
  ...
  async def compute(self):
    ...
    if isinstance(self._value, executor_value_base.ExecutorValue):
      ...
      results = await asyncio.gather(*[
          ComposingFederatingStrategyValue(v, t).compute()
...
class ComposingFederatingStrategy(federating_executor.FederatingStrategy):
  """A strategy for resolving federated types and intrinsics in disjoint scopes.

  This strategy implements the `FederatingStrategy` interface and provides
  logic for resolving federated types and federated intrinsics on an underlying
  collection of target executors. This strategy can be used in hierarchical
  aggregation structures with federating executors resolving disjoint scopes at
  the leafs.
  ...
  * `tff.CLIENTS`
  ...
  @classmethod
  def factory(cls, server_executor, target_executors):
  ...
  def __init__(self, executor: federating_executor.FederatingExecutor,
    ...
    """Creates a `ComposingFederatingStrategy`.
  ...
  @property
  def value_type(self):
    return ComposingFederatingStrategyValue
  ...
  async def create_federated_value(self, value, type_spec):
    if type_spec.placement == placement_literals.SERVER:
      ...
      return ComposingFederatingStrategyValue(results, type_spec)
    elif type_spec.placement == placement_literals.CLIENTS:
      if type_spec.all_equal:
        results = await asyncio.gather(
            *[c.create_value(value, type_spec) for c in self._target_executors])
        return ComposingFederatingStrategyValue(results, type_spec)
      else:
        py_typecheck.check_type(value, list)
        ...
        for child, num_clients in zip(self._target_executors, cardinalities):
          ...
          offset = new_offset
        return ComposingFederatingStrategyValue(await asyncio.gather(*results),
                                                type_spec)
    else:
  ...
  async def _eval(self, arg, intrinsic, placement, all_equal):
    ...
    result_type = computation_types.FederatedType(
        fn_type.result, placement, all_equal=all_equal)
    return ComposingFederatingStrategyValue(result_vals, result_type)
  ...
  async def _map(self, arg, all_equal=None):
    ...
    return ComposingFederatingStrategyValue(result_vals, federated_type)
  ...
  async def create_federated_mean(self, arg):
    ...
    async def _create_total():
      total = await self.create_federated_sum(arg)
    ...
    return ComposingFederatingStrategyValue(result, type_signature)

  @tracing.trace
  async def create_federated_reduce(self, arg):
    raise NotImplementedError('The secure reduce intrinsic is not implemented.')
  ...
  async def create_federated_sum(self, arg):
    ...
    return await self.create_federated_aggregate(aggregate_args)
  ...
  async def create_federated_zip_at_clients(self, arg):
    ...
    return ComposingFederatingStrategyValue(result, result_type)
