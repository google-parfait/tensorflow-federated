...
class CachingExecutorTest(absltest.TestCase):
  ...
  def test_with_selection_by_index(self):
    ...
    self.assertEqual(str(v1.identifier), '1')
    v2 = loop.run_until_complete(ex.create_selection(v1, 0))
    self.assertEqual(str(v2.identifier), '1[0]')
    v3 = loop.run_until_complete(ex.create_selection(v1, 1))
    self.assertEqual(str(v3.identifier), '1[1]')
    v4 = loop.run_until_complete(ex.create_selection(v1, 0))
    self.assertIs(v4, v2)
    v5 = loop.run_until_complete(ex.create_selection(v1, 1))
    self.assertIs(v5, v3)
