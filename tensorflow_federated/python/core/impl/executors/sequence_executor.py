...
class _SyncFromAsyncIterable(object):
  ...
  def __init__(self, async_iterable):
    ...
    self._thread = threading.Thread(
        target=functools.partial(_run_loop, self._event_loop), daemon=True)
    self._thread.start()

    def _finalizer(loop, thread):
      loop.call_soon_threadsafe(loop.stop)
      thread.join()

    weakref.finalize(self, _finalizer, self._event_loop, self._thread)
  ...
  def __iter__(self):
    ...
    def _next(async_iter, event_loop):
      future = asyncio.run_coroutine_threadsafe(async_iter.__anext__(),
                                                event_loop)
    ...
    def _generate(event_loop, async_iter):
      try:
        while True:
          yield _next(async_iter, event_loop)
