...
class DefaultFederatingStrategy(federating_executor.FederatingStrategy):
  ...
  def _get_child_executors(self, placement, index=None):
    child_executors = self._executor._target_executors[placement]  # pylint: disable=protected-access
  ...
  async def federated_mean(self, arg):
    # pylint: disable=protected-access
    arg_sum = await self._executor._compute_intrinsic_federated_sum(arg)
    # pylint: enable=protected-access
  ...
  async def federated_sum(self, arg):
    ...
    zero, plus = await asyncio.gather(
        ...
        executor_utils.embed_tf_binary_operator(self._executor,
                                                ...
                                                tf.add))
    # pylint: disable=protected-access
    return await self._executor._compute_intrinsic_federated_reduce(
        federating_executor.FederatingExecutorValue(
            anonymous_tuple.AnonymousTuple([
