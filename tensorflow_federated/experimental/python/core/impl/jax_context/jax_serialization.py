...
def _make_xla_binding_for_type(type_spec):
  ...
  Args:
    type_spec: The type to generate the binding for.
  ...
  if type_spec is None:
    ...
    def _make_starting_at_index(type_spec, idx):
      if isinstance(type_spec, computation_types.TensorType):
        ...
        return pb.Xla.Binding(struct=pb.Xla.StructBinding(
...
def serialize_jax_computation(traced_func, arg_func, parameter_type,
                              context_stack):
  """Serializes a Python function containing JAX code as a TFF computation.

  Args:
    traced_func: The Python function containing JAX code to be traced by JAX and
      serialized as a TFF computation containing XLA code.
    arg_func: An unpacking function that takes a TFF argument, and returns a
      combo of (args, kwargs) to invoke `traced_func` with (e.g., as the one
      constructed by `function_utils.create_argument_unpacking_fn`).
  ...
  """
  py_typecheck.check_callable(traced_func)
  py_typecheck.check_callable(arg_func)
