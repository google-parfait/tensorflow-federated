...
class ClientFedAvg(optimizer_utils.ClientDeltaFn):
  ...
  def __init__(
    ...
    Args:
      ...
      optimizer: A `optimizer_abc.Optimizer` instance, or a function returns
...
def build_federated_averaging_process(
  ...
  *   `get_model_weights`: A `tff.Computation` that takes as input the
  ...
  Args:
    ...
    client_optimizer_fn: A `optimizer_abc.Optimizer`, or a no-arg callable that
      returns a `tf.keras.Optimizer`.
    server_optimizer_fn: A `optimizer_abc.Optimizer`, or a no-arg callable that
      returns a `tf.keras.Optimizer`.
