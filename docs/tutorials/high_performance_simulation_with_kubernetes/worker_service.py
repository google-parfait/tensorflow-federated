...
class _EMNISTPartitionDataBackend(tff.framework.DataBackend):
  ...
  def __init__(self):
    element_spec = collections.OrderedDict([
        ...
        (
            'pixels',
            tf.TensorSpec(shape=(28, 28), dtype=tf.float32, name=None),
        ),
    ])
  ...
  def preprocess(self, dataset: tf.data.Dataset):
    ...
    return (
        dataset.repeat(_NUM_EPOCHS).shuffle(_SHUFFLE_BUFFER, seed=1).map(map_fn)
    )
...
def main(argv: Sequence[str]) -> None:
  ...
  tff.simulation.run_server(
      executor_factory, _THREADS, _PORT, None, _GRPC_OPTIONS
  )
